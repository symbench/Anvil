...
# Run stl2ascii on this.
  def setup_of(self,problemdict):
    ...
    if not os.path.exists(casefolder):
        ...
        log.warning("Casefolder already exists, files will be overwritten")
        #pass
     ...
     def read_force(self):
       try:
         ...
         if F_out: 
           ...
           log.info('List of forces:{}'.format(F_found))
         ...
         log.error("!An exception occurred- Missing result file!") 
...
if __name__ == "__main__":
    ...
    log.info('dex source loc:', dex_source_loc)
           ...
           if key =="meshing":
             if value=='self':
                ...
                for _mesh_ in alt_mesh:
                    log.info('mesh is:',_mesh_)       
                    dex_file_editor.set_dexof_parameters_mesh(data,_mesh_)                    
                    result_folder=foam_sim.run()
                    _result_reader_= read_results(result_folder)
                    F= _result_reader_.read_force()
                    log.info('mesh size is {} and found drag force is {}'.format(_mesh_,Fd))
                    if (F[0]<default_max):
                       break
