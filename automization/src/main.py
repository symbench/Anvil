...
if __name__=='__main__':
           ...
           if key =="meshing":
             if value=='self':
                ...
                for _mesh_ in alt_mesh:
                    ...
                    _result_reader_= read_results(result_folder)
                    F= _result_reader_.read_force()
                    print('mesh size is {} and found drag force is {}'.format(_mesh_,F))
                    if (F[0]<default_max):
                       break
        return F
     ...
     def run_cad_cfd(X):
        ...
        print('data file name:',data_file_name, 'data is:',np.append(X,force))
     ...
     def run_cad_cfd_opt_target(X,target):
        print("X is:",X,'variables are:',variables_to_optimize)
        cad_obj.set_parameter_by_value(variables_to_optimize,X)
        cad_obj.create_stl()
        #force=run_cfd()
        ...
        if opt_target=="lift":
          out=-1*force[1]
        elif opt_target=="drag":
          out= force[0]
        elif opt_target=="lift_drag_ratio":
          out= -1*force[1]/force[0]
        print('data file name:',data_file_name, 'data is:',np.append(X,out))
     ...
     if mode=="optimization":
        ...
        opt_target= data["optimizer"]["target"]
        
      
        print('optimization method is:',optimization_method,'Budget is:',budget,'Aq func:',aq_func,'opt target:',opt_target) 
